...
def get_item_project_name(item):
    """
    兼容两种原子池格式：
    1. 新格式：item["project_name"]
    2. 旧格式：item["project"]["name"]

    优先使用顶层 project_name，避免新原子池被误判为“未分类项目”。
    """
    ...
    obj_name = (project_obj.get("name") or "").strip()
    if obj_name:
        return obj_name
...
def get_item_project_name_source(item):
    project_obj = item.get("project", {}) or {}
    return (
        item.get("project_name_source")
        or project_obj.get("name_source")
        or "explicit"
    )
...
def atomic_item_to_tree_entry(item):
    ...
    return {
        ...
        "project_name_source": get_item_project_name_source(item),
        "member": {
            "uid": member.get("uid", ""),
            "id": member.get("id", ""),
...
5. 严禁将 subtopic 写入 project_name。例如输入 project_name 是“WIS AI Defect识别系统”，输出 project_name 也必须是“WIS AI Defect识别系统”，不能改成“Defect bot”“LGCFA产品上线”“trunkspot算法”等正文推测名称。
...
# 层级修复要求
如果校验发现 project/subtopic 混淆，必须按以下方式修复：
- Platform 使用原输入 platform
- Project 使用原输入 project_name
- Subtopic 只能作为 Project 下一级
- 不允许把 subtopic 提升为 project
- 不允许出现“未分类项目”，除非原输入确实没有 project_name
- mention 必须恢复为完整 `[@姓名](mention:uid:id)`
...
def generate_card_content(project, long_markdown, week_info=None):
    ...
    # 默认喂入完整周报；如担心超长导致模型超时，可配置 weekly_card_input_max_chars > 0 进行截断。
    ...
    try:
        llm_result = call_llm(
            messages=[
                {"role": "system", "content": "你是内容整理助手，请综合完整周报生成适合飞书卡片展示的精炼正文，不要输出代码块。"},
