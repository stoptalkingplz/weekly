...
4. 使用豆包/OpenAI SDK 将 person_blocks 结构化为临时 weekly items。
...
def llm_extract_items_from_person_blocks(person_blocks, project):
    ...
    # 用规则解析出的 person_blocks 对 LLM 抽取结果做覆盖兜底，避免 timeline 少内容。
...
def get_or_create_weekly_note(project, week_info):
    ...
    if not target_project_guid or not target_folder_guid:
        raise ValueError("未配置 weekly_note_guid，且缺少 weekly_target_project_guid / weekly_target_parent_guid，无法创建周报")
